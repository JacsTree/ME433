...
bool timer_interrupt_function(__unused struct repeating_timer *t) {
    ...
    if (i>=500) pwm_set_gpio_level(PWMPIN, i);
    i=i+50;
    if (i>2500) {i=300;pwm_set_gpio_level(PWMPIN, 500);}
    return true;
}
...
int main()
{
    ...
    float div = 150; // must be between 1-255
    pwm_set_clkdiv(slice_num, div); // sets the clock speed
    uint16_t wrap = 19999; // when to rollover, must be less than 65535
    pwm_set_wrap(slice_num, wrap); 
    pwm_set_enabled(slice_num, true); // turn on the PWM

    //pwm_set_gpio_level(PWMPIN, 500); // set the duty cycle to 50% 500-2500

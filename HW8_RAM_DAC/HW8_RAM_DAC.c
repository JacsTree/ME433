...
int main() {
    init();
    uint8_t buffer[3];
    buffer[0] = 0b00000011; buffer[1] = 0x00; buffer[2] = 0x00;
    uint8_t buf[2];
    uint16_t addr = 0;
    while (true) {
        buffer[1] = addr >> 8;
...
static inline void spi_ram_init(){
    uint8_t buft[2];
    buft[0]=0b00000001;
    buft[1] = 0b01000000; //setup sequential
    cs_select_ram();
    spi_write_blocking(spi_default, buft, 2);
    cs_deselect_ram();

    uint16_t voltages_1[1000]; 
    ...
    buffer[2] = 0x00;
    for(int i=0;i<1000;i++){
        buffer[i*2+3] = 0b00110000|(0b00001111 & (voltages_1[i] >> 6));
